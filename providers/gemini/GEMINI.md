...
Workflow: plan-only, then marker-pass-only. No file edits before explicit approval. After approval, inspect repo, insert tiny `TODO(chisel:item-N) CHISEL:<session-id>` markers using language-native comments, save minimal `.chisel/<session-id>.md` and `.chisel/<session-id>.json` receipts with "markers only" status, then stop before implementation. User then runs inline completion or implements by hand and reviews every generated line.
