...
Workflow: plan, approval, repo inspection, tiny `TODO(chisel:item-N) CHISEL:<session-id>` markers using language-native comments, minimal `.chisel/<session-id>.md` note, stop before implementation. User then runs inline completion and reviews every generated line.
