...
function writeSessionChoices(command, summaries, out = process.stdout) {
  out.write('Multiple Chisel sessions found in this repo. Pass one explicitly:\n');
  for (const summary of summaries) {
    out.write(`  ${summary.sessionId}  ${summary.task}  ${summary.fileCount} file${summary.fileCount === 1 ? '' : 's'}\n`);
  }
  out.write(`Run \`chisel ${command} <session-id>\`.\n`);
}
...
function resolveSessionId(opts, out = process.stdout, command = 'status') {
  ...
  if (!summaries.length) {
    out.write('No Chisel sessions found in this repo.\n');
    return null;
  }
  if (summaries.length === 1) {
    out.write(`no session id given, using latest: ${summaries[0].sessionId}\n`);
