...
function verifySession(opts, out = process.stdout) {
  ...
  if (!sessionId) throw new Error(`verify requires a session id\n\n${usage()}`);
  ...
  for (const marker of familyMarkers.length ? familyMarkers : sessionMarkers) {
  ...
  if (fs.existsSync(path.join(opts.target, '.git')) && filesTouched.length) {
    ...
    nonMarkerChanges.push(...parseUnifiedDiff(diff));
  }
